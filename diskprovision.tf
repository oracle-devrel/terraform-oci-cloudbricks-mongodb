...
resource "null_resource" "provisioning_disk_config_primary" {
  depends_on = [oci_core_volume_attachment.ISCSIDiskAttachment_config_primary]
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "set +x",
      "${local.iscsiadm} -m node -o new -T ${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.iqn} -p ${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.port}",
      "${local.iscsiadm} -m node -o update -T ${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.iqn} -n node.startup -v automatic",
      "${local.iscsiadm} -m node -T ${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.iqn} -p ${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.port} -l",
    ]
  }
}


resource "null_resource" "partition_disk_config_primary" {
  depends_on = [null_resource.provisioning_disk_config_primary]
  ...
  provisioner "remote-exec" {
    inline = [
      "set +x",
      "export DEVICE_ID=/dev/disk/by-path/ip-${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.port}-iscsi-${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.iqn}-lun-1",
...
resource "null_resource" "pvcreate_exec_config_primary" {
  depends_on = [null_resource.partition_disk_config_primary]
  connection {
    type        = "ssh"
    host        = oci_core_instance.config_primary.private_ip
    user        = "opc"
...
resource "null_resource" "vgcreate_exec_config_primary" {
  depends_on = [null_resource.pvcreate_exec_config_primary]
  connection {
    type        = "ssh"
    host        = oci_core_instance.config_primary.private_ip
    user        = "opc"
  ...
  provisioner "remote-exec" {
    inline = [
      "set +x",
      "export DEVICE_ID=/dev/disk/by-path/ip-${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.port}-iscsi-${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.iqn}-lun-1",
      "${local.vgcreate} ${oci_core_volume_attachment.ISCSIDiskAttachment_config_primary.display_name} $${DEVICE_ID}-part1",
    ]
  }
}


resource "null_resource" "format_disk_exec_config_primary" {
  depends_on = [
    null_resource.vgcreate_exec_config_primary,
    oci_core_volume_attachment.ISCSIDiskAttachment_config_primary
  ]
  connection {
    type        = "ssh"
    host        = oci_core_instance.config_primary.private_ip
...
resource "null_resource" "mount_disk_exec_config_primary" {
  depends_on = [null_resource.format_disk_exec_config_primary,
    oci_core_volume_attachment.ISCSIDiskAttachment_config_primary
  ]
  connection {
    type        = "ssh"
...
resource "null_resource" "provisioning_disk_config_secondary" {
  depends_on = [oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary]
  ...
  provisioner "remote-exec" {
    ...
    inline = [
      "set +x",
      "${local.iscsiadm} -m node -o new -T ${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.iqn} -p ${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.port}",
      "${local.iscsiadm} -m node -o update -T ${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.iqn} -n node.startup -v automatic",
      "${local.iscsiadm} -m node -T ${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.iqn} -p ${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.port} -l",
    ]
  }
}


resource "null_resource" "partition_disk_config_secondary" {
  depends_on = [null_resource.provisioning_disk_config_secondary]
  ...
  provisioner "remote-exec" {
    inline = [
      "set +x",
      "export DEVICE_ID=/dev/disk/by-path/ip-${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.port}-iscsi-${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.iqn}-lun-1",
      "${local.fdisk} $${DEVICE_ID}",
    ]
  }
}
...
resource "null_resource" "pvcreate_exec_config_secondary" {
  depends_on = [null_resource.partition_disk_config_secondary]
  connection {
    type        = "ssh"
    host        = oci_core_instance.config_secondary.private_ip
    user        = "opc"
...
resource "null_resource" "vgcreate_exec_config_secondary" {
  depends_on = [null_resource.pvcreate_exec_config_secondary]
  connection {
    type        = "ssh"
    host        = oci_core_instance.config_secondary.private_ip
    user        = "opc"
  ...
  provisioner "remote-exec" {
    inline = [
      "set +x",
      "export DEVICE_ID=/dev/disk/by-path/ip-${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.ipv4}:${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.port}-iscsi-${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.iqn}-lun-1",
      "${local.vgcreate} ${oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary.display_name} $${DEVICE_ID}-part1",
    ]
  }
}


resource "null_resource" "format_disk_exec_config_secondary" {
  depends_on = [
    null_resource.vgcreate_exec_config_secondary,
    oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary
  ]
  connection {
    type        = "ssh"
    host        = oci_core_instance.config_secondary.private_ip
...
resource "null_resource" "mount_disk_exec_config_secondary" {
  depends_on = [null_resource.format_disk_exec_config_secondary,
    oci_core_volume_attachment.ISCSIDiskAttachment_config_secondary
  ]
  connection {
    type        = "ssh"
